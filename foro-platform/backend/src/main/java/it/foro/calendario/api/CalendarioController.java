...
public class CalendarioController {
  private final TenantContext tenant; private final CalendarioRepository calendari; private final EventoCalendarioRepository eventi; private final CalendarioCondivisioneRepository condivisioni; private final StudioMembershipRepository membership; private final UserAccountRepository utenti; private final InvitoEventoService invitoService;
  public CalendarioController(TenantContext tenant, CalendarioRepository calendari, EventoCalendarioRepository eventi, CalendarioCondivisioneRepository condivisioni, StudioMembershipRepository membership, UserAccountRepository utenti,InvitoEventoService invitoService){this.tenant=tenant;this.calendari=calendari;this.eventi=eventi;this.condivisioni=condivisioni;this.membership=membership;this.utenti=utenti;this.invitoService=invitoService;}
  public record CalendarioResponse(UUID id,String nome,String colore,boolean condivisoTuttoStudio,List<UUID> condivisoCon){}
  ...
  public record CreaCalendarioRequest(@NotBlank @Size(max=160) String nome,@Pattern(regexp="^#[0-9a-fA-F]{6}$") String colore,List<UUID> condivisoCon){}
  ...
  public List<CalendarioResponse> calendari(){
    var tutti=calendari.findAllByStudioIdOrderByNome(tenant.studioId()); var quote=condivisioni.findAllByCalendarioIdIn(tutti.stream().map(Calendario::getId).toList());
  ...
  public CalendarioResponse creaCalendario(@Valid @RequestBody CreaCalendarioRequest r){
    ...
    condivisioni.saveAll(destinatari.stream().map(id->new CalendarioCondivisione(c.getId(),tenant.studioId(),id)).toList()); return risposta(c,destinatari);
  }
  @GetMapping("/eventi")
  ...
  public EventoResponse creaEvento(@Valid @RequestBody CreaEventoRequest r){
    ...
    var calendario=calendari.findByIdAndStudioId(r.calendarioId(),tenant.studioId()).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"CALENDARIO_NON_TROVATO"));
  ...
  private CalendarioResponse risposta(Calendario c,List<UUID> condivisoCon){return new CalendarioResponse(c.getId(),c.getNome(),c.getColore(),c.isCondivisoTuttoStudio(),condivisoCon);}
  ...
  private void verificaAccessoEvento(EventoCalendario evento){var calendario=calendari.findByIdAndStudioId(evento.getCalendarioId(),tenant.studioId()).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"EVENTO_NON_TROVATO"));if(!puoVedere(calendario,condivisioni.findAllByCalendarioIdIn(List.of(calendario.getId()))))throw new ResponseStatusException(HttpStatus.NOT_FOUND,"EVENTO_NON_TROVATO");}
