...
public class CalendarioController {
  ...
  public record EventoResponse(UUID id,UUID calendarioId,UUID creatoreId,String calendarioNome,String colore,String titolo,Instant inizio,Instant fine,String luogo,String note,String partecipanti,String statoDisponibilita,Integer promemoriaMinuti,String categoria,boolean tuttoGiorno,UUID serieId,String ricorrenza,LocalDate fineRicorrenza,UUID praticaId,String praticaCodice,String praticaTitolo,String praticaStato){}
  ...
  public record CreaEventoRequest(@NotNull UUID calendarioId,@NotBlank @Size(max=240) String titolo,@NotNull Instant inizio,@NotNull Instant fine,@Size(max=4000) String note,List<UUID> invitatiIds,@Pattern(regexp="LIBERO|PROVVISORIO|OCCUPATO|FUORI_SEDE") String statoDisponibilita,@Min(0) @Max(40320) Integer promemoriaMinuti,@Size(max=60) String categoria,boolean tuttoGiorno,@Pattern(regexp="NESSUNA|GIORNALIERA|SETTIMANALE|MENSILE") String ricorrenza,LocalDate fineRicorrenza,UUID praticaId){}
  ...
  public EventoResponse creaEvento(@Valid @RequestBody CreaEventoRequest r){
    ...
    if(r.praticaId()!=null)pratiche.verificaCollegabileAgenda(r.praticaId());
    ...
    do {
      evento=eventi.save(new EventoCalendario(tenant.studioId(),calendario.getId(),tenant.userId(),r.titolo().trim(),inizio,fine,null,r.note(),"",Optional.ofNullable(r.statoDisponibilita()).orElse("OCCUPATO"),r.promemoriaMinuti(),r.categoria(),r.tuttoGiorno(),serieId,frequenza,r.fineRicorrenza(),r.praticaId())); occorrenze++;
    ...
    invitoService.invia(tenant.studioId(),evento,Optional.ofNullable(r.invitatiIds()).orElse(List.of())); pratiche.registraEvento(r.praticaId(),evento.getId(),"PRATICA_EVENTO_COLLEGATO"); return risposta(evento,calendario);
  }
  ...
  public EventoResponse aggiornaEvento(@PathVariable("id") UUID id,@Valid @RequestBody CreaEventoRequest r){
    ...
    var praticaPrecedente=evento.getPraticaId();
    ...
    if(r.praticaId()!=null)pratiche.verificaCollegabileAgenda(r.praticaId());
    evento.aggiorna(calendario.getId(),r.titolo().trim(),r.inizio(),r.fine(),r.note(),Optional.ofNullable(r.statoDisponibilita()).orElse("OCCUPATO"),r.promemoriaMinuti(),r.categoria(),r.tuttoGiorno(),r.praticaId());
    if(praticaPrecedente!=null&&!praticaPrecedente.equals(r.praticaId()))pratiche.registraEvento(praticaPrecedente,id,"PRATICA_EVENTO_SCOLLEGATO");
    pratiche.registraEvento(r.praticaId(),id,"PRATICA_EVENTO_MODIFICATO");
    return risposta(evento,calendario);
  }
  @DeleteMapping("/eventi/{id}") @ResponseStatus(HttpStatus.NO_CONTENT) @Transactional
  public void eliminaEvento(@PathVariable("id") UUID id){var evento=eventi.findByIdAndStudioId(id,tenant.studioId()).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"EVENTO_NON_TROVATO"));verificaAccessoEvento(evento);pratiche.registraEvento(evento.getPraticaId(),id,"PRATICA_EVENTO_ELIMINATO");eventi.delete(evento);}
